static const char rcsid[] = "$Rxvt: grxlib.c,v 1.2 1998/04/20 07:26:20 mason Exp $";
...
InitializeGraphics (int scroll_text_up)
{
   ...
   /* query wterm to find if graphics are available */
   fflush (stdout);
   printf ("\033GQ");
   fflush (stdout);
   while (1)
	...
	if ((fgets (line, LINESZ, infd) != NULL) &&
	    (sscanf (line,"\033G%d", &i) == 1))
	  {
	     if (!i)
	       {
		  fprintf (stderr, "wterm graphics not available\n");
		  CloseGraphics ();
		  return 0;
	       }
	     break;
